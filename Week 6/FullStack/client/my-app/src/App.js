...
function App() {
  ...
  const addEmployee = ((newEmployee) => {
    axios.post("http://localhost:3000/post", newEmployee)
        .then(res => {
  ...
  const deleteEmployee = ((employeeId) => {
    axios.delete(`http://localhost:3000/delete/${employeeId}`)
        .then(res => {
  ...
  const editEmployee = ((updates, employeeId) => {
    axios.put(`http://localhost:3000/edit/${employeeId}`, updates)
        .then(res => {
